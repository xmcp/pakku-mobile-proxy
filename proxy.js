...
var pac_content=fs.readFileSync('proxy.pac',{encoding: 'utf-8'}).replace(/__SETTINGS__/g,JSON.stringify({
    ip: settings.ip,
...
proxy_modify.on('proxyRes', function(proxyRes, req, res) {
    ...
    proxyRes.on('end', function () {
        console.log('GOT DANMAKU RESPONSE',req.url);
...
http_server.on('connect',function(req,c,head) {
    ...
    if(req.url!=='api.bilibili.com:443')
        return;
...
console.log(`proxy url: http://${settings.ip}:${settings.http_port}/proxy.pac`);
console.log(`cert download: http://${settings.ip}:${settings.http_port}/cert.pem`);